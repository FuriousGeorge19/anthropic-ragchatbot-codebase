...
The project uses several code quality tools to maintain consistent code style and catch potential issues:
...
- **mypy**: Static type checker (informational warnings)
...
**Configuration Files:**
- `pyproject.toml`: Contains [tool.black], [tool.isort], and [tool.mypy] sections
- `.flake8`: Flake8 configuration (line length, ignore rules, exclusions)

**Pre-commit Workflow:**
1. Run `./format_code.sh` to auto-fix formatting issues
2. Run `./check_quality.sh` to verify all checks pass
3. Fix any remaining flake8 issues manually
4. Commit your changes

**Note:** The quality check script will fail if Black or flake8 finds issues, but mypy warnings are informational only and won't block commits.
